 ...
 */
wx.getUserInfo = function (code, callback, force, withToken) {
  if(!force && oauthAccessToken.access_token && !isTokenExpired()) {
    debug('using existing access token');
    getUserInfoWithToken(oauthAccessToken, callback, withToken);
    return;
  }
  wx.getOAuthAccessToken(code, function(tokenInfo) {
    getUserInfoWithToken(tokenInfo, callback, withToken);
  });
};
 ...
 *   callback: function(data) {} //callback to call after getting profile,
 *                               //with token info merged based on the 'returnToken' option above
 * }
 * @param options
 */
wx.getUserInfoByOptions = function(options) {
  wx.getUserInfo(options.code, options.callback, options.force, options.returnToken);
};
...
function getUserInfoWithToken(tokenInfo, callback, withToken) {
  util.sendWechatRequest({
    ...
    if(callback) {
      withToken ? callback(_.assign({}, tokenInfo, data)) : callback(data);
    }
  });
}
