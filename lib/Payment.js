...
class Payment {
  ...
  generateSignature (params, signType, sandbox) {
    const data = this.generateGeneralPaymentSignature(params, signType || SIGN_TYPE.MD5, sandbox);
  ...
  generateGeneralPaymentSignature(params, signType, sandbox) {
    ...
    const keys = originalKeys.filter((key) => {
      ...
      return (typeof key === 'string') && val !== undefined && val !== '' && val !== null;
    });
    const newParams = {};
    keys.forEach((key) => {
    ...
    // console.log(params);
    console.log(str);
    let paySign = (signType === SIGN_TYPE.HMAC_SHA256)
   ...
   */
  closeOrder (orderId) {
    return this.simpleRequest(this.paymentUrls.CLOSE_ORDER, {out_trade_no: orderId})
      .catch((reason) => {
  ...
  downloadBill (billDate, billType, noGzip) {
    const data = this.generateSignature(this.mergeParams({
      bill_date: billDate,
      bill_type: billType,
  ...
  downloadFundFlow (billDate, accountType, noGzip) {
    const data = this.generateSignature(this.mergeParams({
      ...
      tar_type: noGzip ? '' : 'GZIP',
    }, SIGN_TYPE.HMAC_SHA256), SIGN_TYPE.HMAC_SHA256);
    return this.download(data, {
      agentOptions: this.getAgentOptions(),
      gzip: !noGzip,
    });
  ...
  getSandboxSignKey () {
    ...
    const data = this.generateSignature(params, true);
    return utils.buildXML(data)
      .then(xmlData => {
  ...
  getAPISignKey (getSandboxKey) {
    return (getSandboxKey || !this.wechatConfig.paymentSandBox) ? this.wechatConfig.paymentKey : this.wechatConfig.paymentSandBoxKey;
  }
  ...
  decryptRefundNotifyResult (xmlResult) {
    return utils.parseXML(xmlResult)
      .then(data => {
        ...
        const bytes = CryptoJS.AES.decrypt(
          cipherParams, CryptoJS.enc.Hex.parse(utils.createBuffer(md5Key).toString('hex')),
          {
