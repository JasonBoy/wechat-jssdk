...
class Payment {
  ...
  /**
   * wechat international merchant only
   * retrieve foreign currency settlements within the specified date range
   * @param {object} query - visit https://pay.weixin.qq.com/wiki/doc/api/external/jsapi.php?chapter=9_14&index=9 for more info
   * @return {Promise}
   */
  settlementQuery(query) {
    return this.simpleRequest(this.paymentUrls.SETTLEMENT_QUERY, query).catch(
      reason => {
        debug('settlement query failed!');
        return Promise.reject(reason);
      }
    );
  }
   ...
   * @param {object} query - visit https://pay.weixin.qq.com/wiki/doc/api/external/jsapi.php?chapter=9_15&index=10 for more info
   * @return {Promise}
   */
  queryexchagerate(query) {
    return this.simpleRequest(
      this.paymentUrls.QUERY_EXCHANGE_RATE,
      {
        ...
        nonce_str: null,
        sign_type: null,
      } // this endpoint would throw errors if nonce_str and sign_type are included in request
    ).catch(
      reason => {
