...
class WechatJSSDK {
  ...
  signSignature (newSignConfig) {
    ...
    if (!window.wx) {
      this.log('wechat js not defined', 'warn');
      return this;
    }
    const wx = window.wx;
    this.wx = wx;
    ...
    wx.ready(() => {
      this.log('wx.ready() done!', 'log', true);
      config.success && config.success.call(this, undefined, wx);
    });

    wx.error((err) => {
      this.log('sign error: ' + JSON.stringify(err), 'error', true);
      config.error && config.error.call(this, err, wx);
    });
  ...
  loadScript () {
    ...
    ele.async = true;
    ele.onload = () => {
      this.log('Wechat script loaded successfully!');
      //init the wechat config
      this.signSignature();
    };
    ele.onerror = (err) => {
      this.log('Failed to load wechat script!', 'error');
      this.log(JSON.stringify(err), 'error', true);
    };
  ...
  callWechatApi (apiName, config) {
    const debug = this.debug;
    if (!apiName) {
      debug && alert('wechat api not specified!');
      return this;
    }
    ...
    try {
      ...
      this.log('call wechat api failed: ', 'error');
      this.log(JSON.stringify(e), 'error');
    }
    return this;
  };

  /**
   * simple wechat-jssdk logger
   * @param {String} msg
   * @param {String} level log method in 'console' object
   * @param {boolean} alertMsg add additional alert invoke
   */
  log(msg, level, alertMsg) {
    if(!this.debug) return;
    const logger = console[level] || console.log;
    const message = '[wechat-jssdk]-' + msg;
    logger(message);
    if(alertMsg) window.alert(message);
  }
}
