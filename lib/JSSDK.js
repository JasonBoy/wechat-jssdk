...
class JSSDK {
  ...
  constructor (options) {
    ...
    setInterval(() => this.refreshedTimes = 0, options.clearCountInterval || 1000 * 7200);
  }
   ...
   */
  static filterSignature (originalSignatureObj) {
  ...
  getAccessToken () {
    ...
    return utils.getGlobalAccessToken(cfg.appId, cfg.appSecret, cfg.accessTokenUrl);
  }
    ...
    return this.store.getGlobalToken()
      .then((globalToken) => {
        if(!globalToken || !globalToken.accessToken || JSSDK.isTokenExpired(globalToken.modifyDate)) {
          debug('global access token was expired, getting new global access token and ticket now...');
  ...
  saveSignature (info) {
    ...
    return this.store.isSignatureExisting(signature.url)
      .then((existing) => {
        if(existing) {
  ...
  getSignature (url, forceNewSignature) {
    url = JSSDK.normalizeUrl(url);
    return this.store.getSignature(url)
      .then((signature) => {
        if(!forceNewSignature && signature && !JSSDK.isTokenExpired(signature.modifyDate)) {
          signature = JSSDK.filterSignature(signature);
  ...
  createSignature (url) {
    return this.prepareGlobalToken()
      .then((data) => {
        const ret = JSSDK.generateSignature(url, data.accessToken, data.jsapi_ticket);
        ret.signatureName = ret.url;
        return this.saveSignature(ret);
      })
      .then(sig => JSSDK.filterSignature(sig))
      ;
  }
  ...
  getCachedSignature (url) {
    ...
    return this.store.getSignature(url)
      .then((signature) => {
        return Promise.resolve(JSSDK.filterSignature(signature));
      });
  }
