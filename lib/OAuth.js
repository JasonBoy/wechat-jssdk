...
class OAuth {
   ...
   * @param {boolean=} isBaseScope , get base info api, default false which gets userinfo
   * @return {string} generated oauth uri
   */
  generateOAuthUrl (myRedirectUrl, isBaseScope) {
    ...
    tempOAuthParams.scope = isBaseScope ? oauthScope.BASE : oauthScope.USER_INFO;
  ...
  getAccessToken (code, key) {
    ...
    return this.store.getOAuthAccessToken(key)
      .then((tokenInfo) => {
