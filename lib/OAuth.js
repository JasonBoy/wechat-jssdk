...
class OAuth {
  ...
  constructor (options) {
    ...
    setInterval(() => this.refreshAccessToken(), options.refreshAccessTokenInterval || 1000 * 60 * 158);
  }
   ...
   * @return {Promise}
   */
  getUserInfoWithToken (tokenInfo, withToken) {
  ...
  getUserInfo (code, withToken) {
    return this.getOAuthAccessToken(code)
      .then((tokenInfo) => {
        return this.getUserInfoWithToken(tokenInfo, withToken);
      })
      .catch((reason) => {
        return Promise.reject(reason);
      })
