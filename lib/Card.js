...
class Card extends BaseConfig {
  ...
  getGlobalToken(force) {
    ...
    if (force) {
      return utils.getGlobalAccessToken
        .call(this, cfg.appId, cfg.appSecret, cfg.accessTokenUrl)
        .then(globalToken => {
          const info = {
            modifyDate: new Date(),
  ...
  sendDecodeRequest(encryptCode, qs) {
    return this.httpClient.sendWechatRequest(
      this.wechatConfig.decodeCardCodeUrl,
      {
        query: qs,
        method: 'POST',
        json: true,
