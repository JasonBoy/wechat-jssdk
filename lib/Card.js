...
class Card {
  ...
  getGlobalToken(force) {
    ...
    return this.store.getGlobalToken()
      .then(globalToken => {
        if(!globalToken || !globalToken.accessToken || utils.isExpired(globalToken.modifyDate)) {
  ...
  getApiTicket () {
    return this.store.getCardTicket()
      .then(ticketInfo => {
        if (ticketInfo && ticketInfo.ticket && !utils.isExpired(ticketInfo.modifyDate)) {
  ...
  sendDecodeRequest(encryptCode, qs) {
    return utils.sendWechatRequest({
      url: this.wechatConfig.decodeCardCodeUrl,
      qs: qs,
      method: 'POST',
      json: true,
      body: {
  ...
  /**
   * Decode card encrypt_code to get real card code
   * @param {string} encryptCode
   * @return {Promise}
   */
  decodeCardCode(encryptCode) {
    return this.getGlobalToken()
      .then(info => {
