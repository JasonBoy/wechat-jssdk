...
class MongoStore extends Store {
  ...
  getCardTicket () {
    ...
    return this.CardTicket.findOne()
      .then(cardTicket => {
  ...
  updateCardTicket (ticketInfo) {
    //Update to DB
    debug('updating card_ticket...');
    this.CardTicket.findOneAndUpdate({}, ticketInfo, {new: true, upsert: true})
      .then(() => {
  ...
  flushCardTicket () {
    ...
    if(!isEmpty(ct)) {
      return this.CardTicket.findOneAndUpdate({}, ct, {new: true, upsert: true})
        .then(() => {
          debug('global token flushed!');
          // Promise.resolve(newResult);
        });
