...
class MongoStore extends Store {
  ...
  updateCardTicket(ticketInfo) {
    ...
    this.CardTicket.updateOne({}, Object.assign({}, ticketInfo), {
      new: true,
      upsert: true,
    }).then(() => {
  ...
  updateSignature(url, newInfo) {
    this.Signature.updateOne({ url }, Object.assign({}, newInfo), {
      upsert: true,
    })
      .then(() => {
  ...
  updateOAuthAccessToken(key, newInfo) {
    this.OAuthToken.updateOne({ key }, Object.assign({}, newInfo), {
      upsert: true,
    })
      .then(() => {
  ...
  flushGlobalToken() {
    ...
    // console.log('gt: ', gt);
    if (!isEmpty(gt)) {
      return this.GlobalToken.updateOne(
        {
          gid: GID,
        },
        Object.assign({}, gt),
