...
mongoStore2.on('initialized', function () {
  ...
  const mockSignature = {
    "jsapi_ticket" : "kgt8ON7yVITDhtdwci0qeX1SvqeQa19KqAy_FSXg0gdhb70VCiKAcZAuaBPolQ5MrotbgGTfKvtOVmcuQ2I4GA",
    "nonceStr" : "xxxxx94fufvbmhteeam0",
    "timestamp" : "you",
    "url" : mockUrl,
    "signature" : "decc3f47253e4408c1cab0bc1a52ae4edbc6f9b4",
    "accessToken" : "Yfi8sx-m1CPnPA7mjw-UpCNFiRgW_LZl8YMoGyEy0l8I7qPhgJwQCTGWF2LUhZdbRpfZIDI-Tq597LXp7zf0DSz2Wcvsjan9zfZw_rewWczX0fpIM4aE7nPC8nr4g92zNSSeAAAHMM",
  ...
  describe('MongoStore2', function () {
    ...
    it('should save new signature to mongodb', function (done) {
      mongoStore2.saveSignature(mockUrl, mockSignature)
        .then(sig => {
    ...
    it('should get the saved signature', function (done) {
      setTimeout(() => {
        mongoStore2.getSignature(mockUrl)
          .then(sig => {
            sig.should.have.property('jsapi_ticket');
    ...
    it('should save new oauth access token to mongodb', function (done) {
      mongoStore2.saveOAuthAccessToken(mockOAuthToken.key, mockOAuthToken)
        .then(sig => {
    ...
    it('should update signature to mongodb', function (done) {
      ...
      setTimeout(() => {
        mongoStore2.updateSignature(mockUrl, newSig)
          .then(sig => {
    ...
    it('should get saved oauth access token', function (done) {
      setTimeout(() => {
        mongoStore2.getOAuthAccessToken(mockOAuthToken.key)
          .then(tokenInfo => {
    ...
    it('should update the oauth access token', function (done) {
      ...
      setTimeout(() => {
        mongoStore2.updateOAuthAccessToken(mockOAuthToken.key, {access_token: newToken})
          .then(newTokenInfo => {
    ...
    it('should save the card ticket', function (done) {
      mongoStore2.updateCardTicket(mockCardTicket)
        .then(cardTicket => {
    ...
    it('should update the card ticket', function (done) {
      const newTicket = Object.assign({}, mockCardTicket, {ticket: util.nonceStr()});
      setTimeout(() => {
        mongoStore2.updateCardTicket(newTicket)
          .then(cardTicket => {
